...
public class ManagerAudio : MonoBehaviour
{
    ...
    private AudioSource musicSourceA, musicSourceB, sfxSource, voiceSourceA, voiceSourceB, voiceSourceC;
    [SerializeField] private float outputMaxVolumeMusic, outputMaxVolumeSFX, outputMaxVolumeVoice;
    private bool musicSourceAIsPlaying;
    #endregion
    ...
    private static ManagerAudio audioManagerInstance;
    public static ManagerAudio AMI
    {
        get
        {
            ...
            {
                audioManagerInstance = FindObjectOfType<ManagerAudio>();
                if (audioManagerInstance == null)
                {
                    audioManagerInstance = new GameObject("Manager_Audio", typeof(ManagerAudio)).GetComponent<ManagerAudio>();
                }
            }
            #endregion
            #endregion
    ...
    private void Awake()
    {
        ...
        DontDestroyMeOnLoad(audioManagerObject);

        musicSourceA = audioManagerObject.AddComponent<AudioSource>();
        musicSourceA.loop = true;
        musicSourceB = audioManagerObject.AddComponent<AudioSource>();
        musicSourceB.loop = true;
        sfxSource = audioManagerObject.AddComponent<AudioSource>();
        voiceSourceA = audioManagerObject.AddComponent<AudioSource>();
        voiceSourceB = audioManagerObject.AddComponent<AudioSource>();
        voiceSourceC = audioManagerObject.AddComponent<AudioSource>();
        maxVolumeMaster = 1;
        ...
        ManagerIO.IOMI.IO_ReadWriteConfigFile("audio", false);  // Read audio config on awake (sets variables)
    }
        ...
        if (audioIsPaused != trueIsPauseFalseIsUnpause)
        {
            ...
            switch (trueIsPauseFalseIsUnpause)
            {
                ...
                case false:
                    ...
                    sfxSource.UnPause();
                    voiceSourceA.UnPause();
                    voiceSourceB.UnPause();
                    voiceSourceC.UnPause();
                    break;
            }
        }
    }
    ...
    public void Audio_VolumeMax_Update()
    {
            ...
            musicSourceA.volume = musicSourceB.volume = outputMaxVolumeMusic;
            sfxSource.volume = outputMaxVolumeSFX;
            voiceSourceA.volume = voiceSourceB.volume = voiceSourceC.volume = outputMaxVolumeVoice;
    }
    #endregion
    #endregion
    ...
    private AudioSource ActiveMusicSource(AudioSource musicSourceSide1, AudioSource musicSourceSide2)
    {
        ...
        if (musicSourceAIsPlaying) { currentSource = musicSourceSide1; }
    ...
    public void AudioMusic_Play(AudioClip musicClip, float volumeBetween0And100 = 100)
    {
        ...
        AudioSource activeSource = ActiveMusicSource(musicSourceA, musicSourceB);
    ...
    {
        AudioSource activeSource = ActiveMusicSource(musicSourceA, musicSourceB);
        AudioSource nextSource = ActiveMusicSource(musicSourceB, musicSourceA);
        musicSourceAIsPlaying = !musicSourceAIsPlaying;
    ...
    {
        volumeBetween0And100 = Audio_Normalise0To100(volumeBetween0And100);
        AudioSource activeSource = ActiveVoiceSource(voiceSourceA, voiceSourceB, voiceSourceC);
